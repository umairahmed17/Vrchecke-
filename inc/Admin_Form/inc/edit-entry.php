...
$first_option    = $entry->get_first_option() ?: 0;
$second_option   = $entry->get_second_option() ?: 0;
$third_option    = $entry->get_third_option() ?: 0;
...
?>
								...
								<tr>
									<th scope="row" class="row-title">
										<label for="tablecell"><?php _e( 'Select first choice:', 'vrchecke' );if ( $first_option ) {
    _e( '&nbsp;Selected', 'vrchecke' );
}?></label>
									</th>
									<td>
										<?php wp_dropdown_pages( array( 'name' => 'first_option', 'post_type' => 'form-companies', 'selected' => (int) $first_option ) );?>
									</td>
								</tr>
								<tr>
									<th scope="row" class="row-title">
										<label for="tablecell"><?php _e( 'Select second choice:', 'vrchecke' );if ( $second_option ) {
    _e( '&nbsp;Selected', 'vrchecke' );
}?></label>
									</th>
									<td>
									<?php wp_dropdown_pages( array( 'name' => 'second_option', 'post_type' => 'form-companies', 'selected' => (int) $second_option ) );?>
									</td>
								</tr>
								<tr>
									<th scope="row" class="row-title">
										<label for="tablecell"><?php _e( 'Select third choice:', 'vrchecke' );if ( $third_option ) {
    _e( '&nbsp;Selected', 'vrchecke' );
}
?></label>
									</th>
									<td>
									<?php wp_dropdown_pages( array( 'name' => 'third_option', 'post_type' => 'form-companies', 'selected' => (int) $third_option ) );?>
									</td>
								</tr>
								<tr>
									<th scope="row" class="row-title">
									...
									<td>									    									    									    									    									    									     <?php if ( $selected_option ) {?>
										<a href="<?php echo get_post_permalink( (int) $selected_option ); ?>"
											id="selected_company_pdf"><?php _e( 'View Selected Company Post', 'vrchecke' )?></a>
										<?php } else {?>
										None Selected
										<?php }?>
									</td>
