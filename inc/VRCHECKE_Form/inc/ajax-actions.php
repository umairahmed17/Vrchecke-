...
function process_form()
{
    ...
    $args_checkbox = $_POST['electrical_eqp'];
    $arg_checkbox  = implode(' | ', $args_checkbox);
    ...
    $args = array(
        ...
        'type_of_devices' => sanitize_text_field($arg_checkbox),
        'default'         => false,
    );
...
function insert_user($user)
{
    ...
    if (!is_wp_error($user['id'])) {
        ...
        // Send email to user
        user_email_send($user, $password = $user['password']);
    }
    return $user['id'];
}
...
function user_email_send(array $user, string $password)
{
    //setting up email
...
function inserting_form_entry($user_id, $args)
{
    ...
    if (!$args['default'] && count($args) === count($default)) {
        ...
        $data  = array(
            'time'              => $now,
            'user_id'           => $user_id,
            'first_echeck'      => (int) $args['first_check'],
            'number_of_devices' => (int) $args['num_devices'],
